subroutine SwashVTKWriteHeader ( uvtk, pstype, nvar, ivtyp, mxkf, mxkl, mykf, mykl )
!
!   --|-----------------------------------------------------------|--
...
!
    ...
    !
    character(6)            :: crform              ! string format for representing layer number
    ...
    character(4)            :: knum                ! string to pass layer number
    character(80)           :: msgstr              ! string to pass message
    character(12)           :: namvar              ! name of output variable
!
!   Structure
!
...
!   Source text
!
    if (ltrace) call strace (ient,'SwashVTKWriteHeader')
    !
    if ( kmax < 10 ) then
       crform = '(i1.1)'
    elseif ( kmax < 100 ) then
       crform = '(i2.2)'
    elseif ( kmax < 1000 ) then
       crform = '(i3.3)'
    else
       crform = '(i4.4)'
    endif
    ...
    vtkline = '  This file was generated by SWASH version '//trim(VERTXT)//'; project: '//trim(PROJID)//'; run number: '//trim(PROJNR)//lf
    ...
    do jvar = 1, nvar
       ...
       if ( ovsvty(ivtype) < 3 ) then        ! scalars
          ...
          if ( ivtype <= 50 ) then
             ...
             !
          elseif ( ivtype <= 70 ) then
             !
             do k = 0, kmax
                !
                write(knum,fmt=crform) k
                namvar = ovsnam(ivtype)(1:len_trim(ovsnam(ivtype)))//knum
                !
                write(intstr(1:12),'(i12)') ioff
                call TXPBLA(intstr,if,il)
                !
    ...
    !
end subroutine SwashVTKWriteHeader
