subroutine SwashVTKPDataSets ( upvt, pstype, nvar, ivtyp, mxk, myk, vtkfile, upvd, psname, iarr )
!
!   --|-----------------------------------------------------------|--
...
!   Source text
!
    if (ltrace) call strace (ient,'SwashVTKPDataSets')
    !
    if ( kmax < 10 ) then
       crform = '(i1.1)'
    ...
    vtkline = '  This file was generated by SWASH version '//trim(VERTXT)//'; project: '//trim(PROJID)//'; run number: '//trim(PROJNR)
          ...
          do iproc = 1, NPROC-1
             ...
             if ( iarr(1,iproc-1) <= iarr(2,iproc-1) .and. iarr(1,iproc) <= iarr(2,iproc) ) then
                if ( iarr(1,iproc) /= iarr(2,iproc-1)-1 ) then
                   write (msgstr, '(a,i3,a,i3)') 'inconsistency found in SwashVTKPDataSets: the end of subdomain ',iproc,' does not match the start of subdomain ',iproc+1
 ...
 15 format(i3.3)
    ...
    write(upvt,'(a)') trim(vtkline)
    !
end subroutine SwashVTKPDataSets
