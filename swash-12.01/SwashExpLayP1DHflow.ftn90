...
!
    ...
    use SwashCommdata3
    use SwashTimecomm
    use m_genarr, only: npu, kup, kpv, work
    use m_parall
...
!
    ...
    if ( iamb == 1 ) then
       ...
       do k = 1, kmax
          ...
          do m = mfu, ml
             ...
             nmd = kgrpnt(md)
             !
             if ( Ubck(nm,kpv(k)) > 1.0e-5 ) then
                !
                ua(nm,k) = u0(nmd,k)
                !
             else if ( Ubck(nm,kpv(k)) < -1.0e-5 ) then
                !
                ua(nm,k) = u0(nm ,k)
                !
             endif
       ...
       enddo
       !
       ! compute horizontal advection term (momentum conservative) at internal u-point (prediction)
       !
       do k = 1, kmax
          !
          do m = mf+1, ml-1
             ...
             if ( wetu(nm) == 1 ) then
                !
                rhsu(nm,k) = rhsu(nm,k) - rdx * ( Ubck(nmu,kpv(k)) * ua(nmu,k) - Ubck(nm,kpv(k)) * ua(nm,k) )
                !
             endif
             !
          enddo
       ...
       do k = 1, kmax
          ...
          do m = mf+1, ml-1
             ...
             if ( wetu(nm) == 1 ) then
                !
                rhsu(nm,k) = rhsu(nm,k) - rdx * ( fac1 * ua(nmu,k) - fac2 * ua(nm,k) )
                !
             endif
             !
          enddo
    ...
    if ( iamb == 1 .and. propsc /= 1 ) then
       ...
       do k = 1, kmax
          ...
          do m = mf+1, ml-1
             ...
             if ( wetu(nm) == 1 ) then
                !
                u1(nm,k) = u1(nm,k) - dt * rdx * ( Ubck(nmu,kpv(k)) * ua(nmu,k)  - Ubck(nm,kpv(k)) * ua(nm,k) )
                !
             endif
             !
          enddo
    ...
    if ( ihydro == 1 .or. ihydro == 2 ) then
       ...
       if ( iamb /= 0 ) then
          !
          ! compute horizontal advection term due to ambient current (momentum conservative and explicit)
          !
       ...
       if ( iamb /= 0 ) then
          ...
          do m = mfu, ml
             ...
             if ( wets(nm) == 1 ) then
                ...
                fac1 = fac * hu(nmd) * fac1 / hs(nm)
                fac2 = fac * hu(nm ) * fac2 / hs(nm)
                !
                if ( propsc == 3 .and. kappa == 1. ) then
                   !
                   ! central differences
...
!TIMG          call SWTSTO(297)
    ...
    !
    ! store layer interfaces in u-point
    !
    zkuo = zku
    !
    ...
    do j = jhk, 3
       ...
       !
       ! compute the mass flux per layer
       !
       do k = 1, kmax
          !
          do m = mf, ml
       ...
       enddo
       !
       ! compute the depth-integrated mass flux
       !
       if ( ihydro /= 3 ) then
          !
          work(:,1) = 0.
       ...
       do m = mfu, mend
          ...
          nmd = kgrpnt(md)
          !
          s1(nm) = s0(nm) - fh * dt * rdx * ( work(nm,1) - work(nmd,1) )
          !
       enddo
       !
       ...
       if ( j <= 2 ) then
          !
          ! correct the water depth for mass flux
          !
          call SwashUpdDepu ( udep )
          !
          ! recompute layer interfaces and layer thicknesses at u-point
          !
          zku(:,0) = zku(:,kmax) + hu(:)
       ...
       do j = 1, 2
          ...
          if ( j == 1 ) then
             ...
             do m = mfu, mend
                ...
                nmu = kgrpnt(mu)
                !
                work(nm,2) = 0.25 * rdx * ( ( Ubck(nmu,1) + Ubck(nm,1) ) * ( so(nmu) + so(nm) ) - ( Ubck(nm,1) + Ubck(nmd,1) ) * ( so(nm) + so(nmd) ) )
                !
             enddo
             !
          else
             ...
             do m = mfu, mend
                ...
                !
                work(nm,2) = 0.25 * rdx * ( ( Ubck(nmu,1) + Ubck(nm,1) ) * ( fac2 + fac ) - ( Ubck(nm,1) + Ubck(nmd,1) ) * ( fac + fac1 ) )
                !
             enddo
             !
          endif
       ...
       do j = 1, 2
          ...
          if ( j == 1 ) then
             ...
             do m = mfu, mend
                ...
                nmu = kgrpnt(mu)
                !
                work(nm,2) = 0.5 * rdx * ( Ubck(nm,1) * ( so(nmu) + so(nm) ) - Ubck(nmd,1) * ( so(nm) + so(nmd) ) )
                !
             enddo
             !
          else
             ...
             do m = mfu, mend
                ...
                fac2 = thetamb * s1(nmu) + (1.-thetamb) * so(nmu)
                !
                work(nm,2) = 0.5 * rdx * ( Ubck(nm,1) * ( fac2 + fac ) - Ubck(nmd,1) * ( fac + fac1 ) )
                !
             enddo
             !
          endif
       ...
       do m = mfu, mend
          ...
          if ( wets(nm) == 1 ) then
             ...
             do k = 1, kpmax-1
                ...
                !
                womp(nm,k) = w1p(nm,k) - ( zksnew(nm,kup(k)) - zks(nm,kup(k)) ) / dt - fac * rdx *( zkuo(nm,kup(k)) - zkuo(nmd,kup(k)) )
                !
             enddo
             !
             womp(nm,0    ) = 0.
             ...
             do kp = kpmax, 1, -1
                ...
                do j = npu(kp), 1, -1
                   ...
                   if ( j == npu(kp) ) then
                      ...
                      !
                      w1(nm,k) = wom(nm,k) + ( zksnew(nm,k) - zks(nm,k) ) / dt + fac * rdx *( zkuo(nm,k) - zkuo(nmd,k) )
                      !
                   endif
                   !
                enddo
