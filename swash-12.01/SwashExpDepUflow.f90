subroutine SwashExpDepUflow ( u1, u0, uvc, qn, quf, q, dq, gmat, rho )
!
!   --|-----------------------------------------------------------|--
...
!   while that for the bottom friction is based on Euler implicit.
!
...
!   Source text
!
    if (ltrace) call strace (ient,'SwashExpDepUflow')
    !
    ...
    do iface = 1, nfaces
       ...
       if ( face(iface)%atti(FMARKER) == 0 ) then   ! internal face
          ...
          if ( wetu(iface) == 1 ) then
             ...
             !
             ! update normal component of flow velocity at face
             !
             u1(iface) = ( u0(iface) - dt * contrib ) / denom
             !
          else
    ...
    if ( ihydro == 1 ) then
       ...
       do iface = 1, nfaces
          ...
          if ( face(iface)%atti(FMARKER) == 0 .and. wetu(iface) == 1 ) then   ! internal wet face
             ...
             icellr = face(iface)%atti(FACECR)
             !
             u1(iface) = u1(iface) - dt*theta*( gmat(iface,1) * dq(icelll) + gmat(iface,2) * dq(icellr) ) / hum(iface)
             !
          else if ( wlimp(iface) .and. wetu(iface) == 1 ) then   ! described water level at boundary face
             !
             ! consider boundary cell of current face
             ...
             icellb = face(iface)%atti(FACEC1)
             !
             u1(iface) = u1(iface) - dt*theta* gmat(iface,1) * dq(icellb) / hum(iface)
             !
          endif
          !
       enddo
       ...
       do icell = 1, ncells
          ...
          if ( wets(icell) == 1 ) then
             !
             w1top(icell) = w1top(icell) + w1bot(icell) + 2.*dt*theta*dq(icell)/hs(icell)
             !
             w1bot(icell) = 0.
             !
             ! loop over faces of the cell
    ...
    do j = jhk, 3
       ...
       !
       ! update the mass flux and compute the water level
       !
       if ( .not.linswe ) then
          !
          do icell = 1, ncells
          ...
          do icell = 1, ncells
             ...
             do jf = 1, cell(icell)%nof
                ...
                ! compute mass flux at current face and add to other faces of the cell
                !
                qn(iface) = lf * dpu(iface) * u1(iface)
                !
                qf = qf + rsgn * qn(iface)
                !
             enddo
    ...
    do iface = 1, nfaces
       ...
       if ( wetu(iface) == 1 ) then                    ! only consider wet face
          ...
          if ( face(iface)%atti(FMARKER) == 0 ) then   ! internal face
             !
             cfl = rdx * dt * ( sqrt(grav*dep) + abs(u1(iface)) )
 ...
 103 format (2x,'the total energy is ',e14.8e2)
    !
end subroutine SwashExpDepUflow
