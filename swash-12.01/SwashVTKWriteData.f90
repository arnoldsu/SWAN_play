subroutine SwashVTKWriteData ( uvtk, pstype, nvar, ivtyp, voqr, data, datak, lenp, mxk, myk, ionod )
!
!   --|-----------------------------------------------------------|--
...
!
    ...
    real*4 , dimension(mxk*myk,0:kmax,*), intent(in) :: datak  ! layer-dependent output data
    !
...
!   Source text
!
    if (ltrace) call strace (ient,'SwashVTKWriteData')
    !
          ...
          elseif ( ivtype <= 70 ) then
             ...
             do k = 0, kmax
                ...
                do j = 1, myk
                   ip = (j-1)*mxk
                   do i = 1, mxk
                      if ( ios == 0 .and. ionod(ip+i) == iproc ) write(uvtk,iostat=ios) datak(ip+i,k,voqr(ivtype))
                   enddo
                enddo
                !
             enddo
          ...
          elseif ( ivtype <= 70 ) then
             ...
             do k = 0, kmax
                ...
                do j = 1, myk
                   ip = (j-1)*mxk
                   do i = 1, mxk
                      if ( ios == 0 .and. ionod(ip+i) == iproc ) write(uvtk,iostat=ios) datak(ip+i,k,voqr(ivtype)), datak(ip+i,k,voqr(ivtype)+1), 0.
                   enddo
                enddo
                !
             enddo
    ...
    !
end subroutine SwashVTKWriteData
