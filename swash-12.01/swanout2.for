...
!
     ...
     &       IVTYPE.NE.90 .AND. IVTYPE.NE.91 .AND.
     &       IVTYPE.NE.95 .AND. IVTYPE.NE.96      ) THEN
           IVSTAT3 = .TRUE.
        ELSE
           IVSTAT3 = .FALSE.
        ENDIF
...
!
      ...
      REAL   , DIMENSION(MXK*MYK,0:KMAX,*) :: VOQK   ! layer-dependent output variables
...
!     determine size of piece of output data
      ...
      ELSE
         ...
         DO IYK = 1, MYK
            ...
            ENDDO
         ENDDO
         IF ( MXKF.GE.MXKL-1 .OR. MYKF.GE.MYKL-1 ) THEN
            MXKF = 1
            MXKL = 0
            MYKF = 1
            MYKL = 0
...
!     create a parallel VTK file
      IF (PARLL.AND.INODE.EQ.MASTER) THEN
...
!        write references to pieces of output data
         CALL SwashVTKPDataSets ( UPVT, PSTYPE, NVAR, IVTYP, MXE, MYE,
...
!     note: stream access is a Fortran 2003 standard
     ...
     &     STATUS='REPLACE', ACCESS='STREAM')
      CALL SwashVTKWriteHeader ( UVTK, PSTYPE, NVAR, IVTYP,
     &                           MXKF, MXKL  , MYKF, MYKL )

!     write appended data to VTK file
      CALL SwashVTKWriteData ( UVTK, PSTYPE, NVAR, IVTYP, VOQR , VOQ,
     &                         VOQK, LEN   , MXK , MYK  , IONOD)
...
!               scalar
     ...
     &                '     '//OVSNAM(IVTYPE)//'              '
              ...
              ELSE IF (OVSVTY(IVTYPE).GT.4) THEN
!               multiple scalar
                NKOLS = OVSVTY(IVTYPE) - 4
                DO N = 1, NKOLS
                   OUTLIN(LINKAR:LINKAR+LFIELD) =
     &                '     '//OVSNAM(IVTYPE)//'              '
                   LINKAR = LINKAR+LFIELD+1
...
!           write units of output quantities
 ...
 110          CONTINUE
            ...
            DO  JVAR = 1, NVAR
              IVTYPE = IVTYP(JVAR)                                        40.31
              IF (OVSVTY(IVTYPE).EQ.3) NKOLS = NKOLS + 1
            ENDDO
...
!                 write second component of a vectorial quantity
     ...
     &                  VOQ(IP,VOQR(IVTYPE)+1)
                ELSE IF (OVSVTY(IVTYPE).GT.4) THEN
                  NKOLS = OVSVTY(IVTYPE) - 4
!                 write other entries of a multiple quantity
                  DO N = 1, NKOLS-1
                     LINKAR = LINKAR + LFIELD + 1
                     WRITE (OUTLIN(LINKAR:LINKAR+LFIELD-1), FMT=FSTR)
     &                     VOQ(IP,VOQR(IVTYPE)+N)
                  ENDDO
                ENDIF
              ENDIF
            ELSE
