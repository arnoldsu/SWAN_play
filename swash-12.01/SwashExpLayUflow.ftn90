...
!
    use ocpcomm4
    use m_genarr, only: hlay, wlimp, work, uwrk
    use SwashCommdata3
    use SwashTimecomm
    use SwashFlowdata
    use SwanGriddata
...
!   Source text
!
    if (ltrace) call strace (ient,'SwashExpLayUflow')
    !
          ...
          do k = 1, kmax
             ...
             do iface = 1, nfaces
                ...
                if ( face(iface)%atti(FMARKER) == 0 .and. wetu(iface) == 1 ) then   ! internal wet face
                   ...
                   icellr = face(iface)%atti(FACECR)
                   !
                   rhsu(iface,k) = rhsu(iface,k) - (1.-theta) * ( gmatu(iface,k,1)*q(icelll,kd) + gmatu(iface,k,2)*q(icelll,k ) + gmatu(iface,k,3)*q(icellr,kd)  &
                                                                + gmatu(iface,k,4)*q(icellr,k ) + gmatu(iface,k,5)*q(icelll,ku) + gmatu(iface,k,6)*q(icellr,ku) )
                   !
                else if ( wlimp(iface) .and. wetu(iface) == 1 ) then   ! described water level at boundary face
                   !
                   ...
                   icellb = face(iface)%atti(FACEC1)
                   !
                   rhsu(iface,k) = rhsu(iface,k) - (1.-theta) * ( gmatu(iface,k,1)*q(icellb,kd) + gmatu(iface,k,2)*q(icellb,k) + gmatu(iface,k,5)*q(icellb,ku) )
                   !
                endif
                !
             enddo
    ...
    if ( ihydro == 1 .or. ihydro == 2 ) then
       ...
       do icell = 1, ncells
          ...
          do k = 1, kmax
             ...
             enddo
             !
             rhsp(icell,k) = rhsp(icell,k) / (dt*theta)
             !
          enddo
          !
       enddo
       ...
       do k = 1, kmax
          ...
          do iface = 1, nfaces
             ...
             if ( face(iface)%atti(FMARKER) == 0 .and. wetu(iface) == 1 ) then   ! internal wet face
                ...
                icellr = face(iface)%atti(FACECR)
                !
                u1(iface,k) = u1(iface,k) - dt*theta * ( gmatu(iface,k,1)*dq(icelll,kd) + gmatu(iface,k,2)*dq(icelll,k ) + gmatu(iface,k,3)*dq(icellr,kd)  &
                ...
                icellb = face(iface)%atti(FACEC1)
                !
                u1(iface,k) = u1(iface,k) - dt*theta * ( gmatu(iface,k,1)*dq(icellb,kd) + gmatu(iface,k,2)*dq(icellb,k) )
                !
             endif
             !
          enddo
             ...
             if ( wets(icell) == 1 ) then
                ...
                if ( ihydro == 1 ) then
                   ...
                   do j = 0, kmax-1
                      ...
                      ku = min(k+j+1,kmax)
                      !
                      w1(icell,k) = w1(icell,k) - dt*theta*( gmatw(icell,k+1,2*j+1)*dq(icell,kd) + gmatw(icell,k+1,2*j+2)*dq(icell,ku) )
                      !
                   enddo
                   !
                else
                   !
                   w1(icell,k) = w1(icell,k) - dt*theta*( gmatw(icell,k+1,1)*dq(icell,kd) + gmatw(icell,k+1,2)*dq(icell,k+1) )
                   !
                endif
                !
             endif
    ...
    do j = jhk, 3
       ...
       do iface = 1, nfaces
          ...
          if ( wetu(iface) == 1 ) then
             ...
             do k = 1, kmax
                !
                uwrk(iface) = uwrk(iface) + qn(iface,k)
                !
             enddo
             !
          endif
       ...
       if ( j == jhk ) then
          ...
          do iface = 1, nfaces
             ...
             if ( wetu(iface) == 1 ) then
                ...
                lf = face(iface)%attr(FACELEN)
                !
                udep(iface) = uwrk(iface) / ( lf * hu(iface) )
                !
             endif
             !
          enddo
       ...
       if ( .not.linswe ) then
          ...
          do icell = 1, ncells
             ...
             do jf = 1, cell(icell)%nof
                ...
                !
                ! compute mass flux at current face and add to other faces of the cell
                !
                qf = qf + rsgn * uwrk(iface)
                !
             enddo
             !
             ! area of cell
             ...
             area = cell(icell)%attr(CELLAREA)
             !
             ! update water level in cell
             !
             s1(icell) = s0(icell) - fh * dt * qf / area
             !
          enddo
          !
       else
          ...
          do icell = 1, ncells
             ...
             do jf = 1, cell(icell)%nof
                ...
                ! compute mass flux at current face and add to other faces of the cell
                !
                qf = qf + rsgn * lf * dpu(iface) * udep(iface)
                !
             enddo
             !
             ! area of cell
       ...
       if ( j <= 2 ) then
          ...
          !
          call SwashUpdUDepu ( udep )
          !
          ! recompute layer interfaces and layer thicknesses at faces
          !
          zku(:,0) = zku(:,kmax) + hu(:)
    ...
    if ( iwvgen /= 0 ) then
       ...
       do icell = 1, ncells
          !
          s1(icell) = s1(icell) + dt * srcm(icell)
          !
       enddo
       !
    endif
          ...
          do icell = 1, ncells
             ...
             if ( wets(icell) == 1 ) then
                ...
                do jf = 1, cell(icell)%nof
                   ...
                   !
                   qf = qf + rsgn * lf * hku(iface,k+1) * u1(iface,k+1)
                   !
                enddo
                !
                ! area of cell
          ...
          if ( wets(icell) == 1 ) then
             ...
             do k = 1, kmax-1
                ...
                do jf = 1, cell(icell)%nof
                   ...
                   !
                   qf = qf + rsgn * lf * zkuo(iface,k) * ( nx * u + ny * v )
                   !
                enddo
                !
                area = cell(icell)%attr(CELLAREA)
    ...
    do iface = 1, nfaces
       ...
       if ( icelll /= 0 .and. icellr /= 0 ) then
          !
          dep = finp * ( s1(icelll) + dps(icelll) ) + (1.-finp) * ( s1(icellr) + dps(icellr) )
          !
       else if ( icelll == 0 ) then
          !
          dep = s1(icellr) + dps(icellr)
          !
       else
          !
          dep = s1(icelll) + dps(icelll)
          !
       endif
       !
       if ( wetu(iface) == 1 ) then                    ! only consider wet face
          ...
          if ( face(iface)%atti(FMARKER) == 0 ) then   ! internal face
             !
             cfl = rdx * dt * ( sqrt(grav*dep) + maxval(abs(u1(iface,:))) )
 ...
 103 format (2x,'the total energy is ',e14.8e2)
    !
end subroutine SwashExpLayUflow
