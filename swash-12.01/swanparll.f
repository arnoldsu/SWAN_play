...
!
      INTEGER      I, IDOM, IENT, IF, IHALO, IL, INB, ISTART,
     &             IX, IXOFF, IY, IYOFF, J, JOFFS(2,4), LENGTH,
     &             MXSIZ, MYSIZ, NNEIGH, NOVLU
...
!     --- in case of stripwise partitioning, extend enclosing box
!         to include inactive part
!         (ensures consistency in size between subdomains)
...
!     --- allocate work arrays for exchanging data
      ...
      ALLOCATE(IWRKEX(MAX(MXC*IHALOX,MYC*IHALOY)))
      IWRKEX = 0
      ALLOCATE(WRKEXG(MAX(MXC*IHALOX,MYC*IHALOY)))
      WRKEXG = 0.
      ALLOCATE(DWRKEX(MAX(MXC*IHALOX,MYC*IHALOY)))
      DWRKEX = 0D0
...
!  3. Method
!
!     Make use of MPI non-blocking recv/send commands
...
!     RBUF  :     recv buffer
!     RRQST :     request handles for the MPI_IRECV command
!     SBUF  :     send buffer
!     SRQST :     request handles for the MPI_ISEND command
!
      INTEGER IDOM, IENT, INB, ISTART, ITAG, J, K, NNEIGH, NOVLU
      INTEGER RRQST(2), SRQST(2)
      REAL RBUF(MAX(MXC*IHALOX,MYC*IHALOY),2),
     &     SBUF(MAX(MXC*IHALOX,MYC*IHALOY),2)
      INTEGER IERR, IF1, IF2, IL1, IL2
      CHARACTER*20 INTSTR, CHARS(2)
      CHARACTER*80 MSGSTR
!
...
!
!     INTSTR           Converts integer to string
!MPI!     MPI_IRECV        Non-blocking MPI receive
!MPI!     MPI_ISEND        Non-blocking MPI send
!MPI!     MPI_WAITALL      MPI wait for communication requests to complete
!     MSGERR           Writes error message
!     STRACE           Tracing routine for debugging
!TIMG!     SWTSTA           Start timing for a section of code
!TIMG!     SWTSTO           Stop timing for a section of code
...
!           post recvs
!           store data to be sent in array SBUF
!           post sends
!
...
!MPI            CALL MPI_IRECV ( RBUF(1,INB), NOVLU, SWREAL, IDOM-1, ITAG,
!MPI     &                       MPI_COMM_WORLD, RRQST(INB), IERR )
!MPI            IF ( IERR.NE.MPI_SUCCESS ) THEN
!MPI               CHARS(1) = INTSTR(IERR)
!MPI               CALL TXPBLA(CHARS(1),IF1,IL1)
!MPI               CHARS(2) = INTSTR(INODE)
...
!           --- store data to be sent in array SBUF
            ...
            DO J = 1, NOVLU
               SBUF(J,INB) = FIELD(KGRPNT(IBLKAD(ISTART+J)),K)
            END DO

!           --- post sends

!MPI            CALL MPI_ISEND ( SBUF(1,INB), NOVLU, SWREAL, IDOM-1, ITAG,
!MPI     &                       MPI_COMM_WORLD, SRQST(INB), IERR )
!MPI            IF ( IERR.NE.MPI_SUCCESS ) THEN
!MPI               CHARS(1) = INTSTR(IERR)
!MPI               CALL TXPBLA(CHARS(1),IF1,IL1)
!MPI               CHARS(2) = INTSTR(INODE)
...
!MPI         CALL MPI_WAITALL(NNEIGH, RRQST, MPI_STATUSES_IGNORE, IERR)
!MPI         IF ( IERR.NE.MPI_SUCCESS ) THEN
!MPI            CHARS(1) = INTSTR(IERR)
!MPI            CALL TXPBLA(CHARS(1),IF1,IL1)
!MPI            CHARS(2) = INTSTR(INODE)
...
!           --- store the received data
            ...
            DO J = 1, NOVLU
               FIELD(KGRPNT(IBLKAD(ISTART+NOVLU+J)),K) = RBUF(J,INB)
            END DO
...
!MPI         CALL MPI_WAITALL(NNEIGH, SRQST, MPI_STATUSES_IGNORE, IERR)
!MPI         IF ( IERR.NE.MPI_SUCCESS ) THEN
!MPI            CHARS(1) = INTSTR(IERR)
!MPI            CALL TXPBLA(CHARS(1),IF1,IL1)
!MPI            CHARS(2) = INTSTR(INODE)
...
!
      INTEGER IDOM, IENT, INB, ISTART, ITAG, J, K, NNEIGH, NOVLU
      INTEGER RRQST(2), SRQST(2)
      INTEGER RBUF(MAX(MXC*IHALOX,MYC*IHALOY),2),
     &        SBUF(MAX(MXC*IHALOX,MYC*IHALOY),2)
      INTEGER IERR, IF1, IF2, IL1, IL2
      CHARACTER*20 INTSTR, CHARS(2)
      CHARACTER*80 MSGSTR
...
!MPI            CALL MPI_IRECV ( RBUF(1,INB), NOVLU, SWINT, IDOM-1, ITAG,
!MPI     &                       MPI_COMM_WORLD, RRQST(INB), IERR )
!MPI            IF ( IERR.NE.MPI_SUCCESS ) THEN
!MPI               CHARS(1) = INTSTR(IERR)
!MPI               CALL TXPBLA(CHARS(1),IF1,IL1)
...
!           --- store data to be sent in array SBUF
            ...
            DO J = 1, NOVLU
               SBUF(J,INB) = IFLD(KGRPNT(IBLKAD(ISTART+J)),K)
            END DO

!           --- post sends

!MPI            CALL MPI_ISEND ( SBUF(1,INB), NOVLU, SWINT, IDOM-1, ITAG,
!MPI     &                       MPI_COMM_WORLD, SRQST(INB), IERR )
!MPI            IF ( IERR.NE.MPI_SUCCESS ) THEN
!MPI               CHARS(1) = INTSTR(IERR)
!MPI               CALL TXPBLA(CHARS(1),IF1,IL1)
...
!           --- store the received data
            ...
            DO J = 1, NOVLU
               IFLD(KGRPNT(IBLKAD(ISTART+NOVLU+J)),K) = RBUF(J,INB)
            END DO
...
!
      INTEGER IDOM, IENT, INB, ISTART, ITAG, J, K, NNEIGH, NOVLU
      INTEGER RRQST(2), SRQST(2)
      REAL*8 RBUF(MAX(MXC*IHALOX,MYC*IHALOY),2),
     &       SBUF(MAX(MXC*IHALOX,MYC*IHALOY),2)
      INTEGER IERR, IF1, IF2, IL1, IL2
      CHARACTER*20 INTSTR, CHARS(2)
      CHARACTER*80 MSGSTR
...
!MPI            CALL MPI_IRECV ( RBUF(1,INB), NOVLU, SWDBLE, IDOM-1, ITAG,
!MPI     &                       MPI_COMM_WORLD, RRQST(INB), IERR )
!MPI            IF ( IERR.NE.MPI_SUCCESS ) THEN
!MPI               CHARS(1) = INTSTR(IERR)
!MPI               CALL TXPBLA(CHARS(1),IF1,IL1)
...
!MPI            CALL MPI_ISEND ( SBUF(1,INB), NOVLU, SWDBLE, IDOM-1, ITAG,
!MPI     &                       MPI_COMM_WORLD, SRQST(INB), IERR )
!MPI            IF ( IERR.NE.MPI_SUCCESS ) THEN
!MPI               CHARS(1) = INTSTR(IERR)
!MPI               CALL TXPBLA(CHARS(1),IF1,IL1)
...
!
     ...
     &              IUT, IVTYPE, IXK, IYK, JVAR, K, LFIELD,
     &              N, NKOLS, NLINES, NREF, NUMDEC, NVAR
      REAL          RVAL1, RVAL2
...
!                --- write second component of a vectorial quantity       40.51
                 WRITE (OUTLIN(IL:IL+LFIELD-1), FMT=FSTR) OVEXCV(IVTYPE)  40.51
              ELSE IF (OVSVTY(IVTYPE).GT.4) THEN                          SWASH
                 NKOLS = OVSVTY(IVTYPE) - 4                               SWASH
!                --- write other entries of a multiple quantity           SWASH
                 DO N = 1, NKOLS-1                                        SWASH
                   IL = IL + LFIELD + 1                                   SWASH
                   WRITE(OUTLIN(IL:IL+LFIELD-1),FMT=FSTR) OVEXCV(IVTYPE)  SWASH
                 END DO                                                   SWASH
              END IF                                                      40.51
            ELSE
              IF ( IVTYPE.LE.50 .OR. IVTYPE.GE.100 ) THEN
...
!
     ...
     &             NREF, NVAR
      REAL         DFAC, FMAX, FTIP1, FTIP2, RVAL1, RVAL2
      LOGICAL      EXIST, OPENED, EQREAL
