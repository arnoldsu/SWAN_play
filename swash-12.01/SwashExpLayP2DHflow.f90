...
!    8.01,   October 2021: Hancock scheme for global continuity equation
...
!
    ...
    use SwashCommdata3
    use SwashTimecomm
    use m_genarr, only: kgrpnt, guu, guv, gvu, gvv, gsqs, gsqsu, gsqsv, msta, mend, nsta, nend, sbimp, slimp, srimp, stimp, npu, kup, kpv, work
    use m_parall
...
!
    ...
    real, dimension(5)                 :: fc       ! energy-conserved weighted interpolation factors for Coriolis force term
    real                               :: fh       ! factor for Hancock/RK3 scheme
    ...
    real                               :: thetamb  ! implicitness factor for ambient current in global continuity eq
...
!
    ...
    enddo
    !
    ! compute the mass flux per width in x-direction
    !
    do k = 1, kmax
       !
       do n = nfu, nl
    ...
    enddo
    !
    ! synchronize mass fluxes at appropriate boundaries in case of repeating grid (if appropriate)
    !
    call periodic ( qx, kgrpnt, 1, kmax )
    !
    ! compute the mass flux per width in x-direction in wl-point based on averaging
    !
    do k = 1, kmax
       !
       do n = nfu, nl
    ...
    enddo
    !
    ! compute the mass flux per width in y-direction
    !
    do k = 1, kmax
       !
       do n = nf, nl
    ...
    !
    ! compute the mass flux per width in y-direction in depth point based on averaging
    !
    do k = 1, kmax
       !
       do n = nf, nl
    ...
    if ( coriolis ) then
       ...
       do n = nfu, nl
          do m = msta(n), mend(n)
             ...
             ndmu = kgrpnt(mu,nd)
             !
             if ( uwetp(nm) == 1 .and. fcor(nm,1) /= 0. ) then
                !
                ! for permanently dry neighbours, corresponding values will be mirrored
                !
                if ( nmu  == 1 ) nmu  = nm
                ...
                if ( ndmu == 1 ) ndmu = ndm
                !
                ! energy-conserved weighting factors according to Espelid et al. (2000)
                !
                fc(1) = sqrt(abs(fcor(nm  ,2))*dpv(nm  ))
                fc(2) = sqrt(abs(fcor(nmu ,2))*dpv(nmu ))
                fc(3) = sqrt(abs(fcor(ndm ,2))*dpv(ndm ))
                fc(4) = sqrt(abs(fcor(ndmu,2))*dpv(ndmu))
                fc(5) = 1. / (4.*sqrt(abs(fcor(nm,1))*dpu(nm)))
                !
                do k = 1, kmax
                   !
                   ! at time level n-1
                   ...
                   !
                   rhsu(nm,k) = rhsu(nm,k) + (0.5+epsab2) * cfu(nm,k)
                   !
                   ! at time level n
                   !
                   ! --- interpolate v-velocity at u-point using energy-conserved weighting factors
                   !
                   v = fc(1) * v0(nm,k) + fc(2) * v0(nmu,k) + fc(3) * v0(ndm,k) + fc(4) * v0(ndmu,k)
                   v = fc(5) * v
                   !
                   cfu(nm,k) = -fcor(nm,1) * v
                   !
                   rhsu(nm,k) = rhsu(nm,k) - (1.5+epsab2) * cfu(nm,k)
                   !
                enddo
                !
             endif
    ...
    !
    ! include effect of ambient current - first order approximation
    !
    if ( iamb == 1 ) then
       !
       ! compute the advective u-velocity in wl-point based on first order upwind
       ...
       call periodic ( ua, kgrpnt, 1, kmax )
       !
       ! compute advection term in x-direction (momentum conservative) at internal u-point (prediction)
       !
       do k = 1, kmax
          !
          do n = nfu, nl
             ...
             do m = mf+1, mlast
                ...
                if ( uwetp(nm) == 1 ) then
                   !
                   rhsu(nm,k) = rhsu(nm,k) - ( Ubck(nmu,kpv(k)) * ua(nmu,k) - Ubck(nm,kpv(k)) * ua(nm,k) ) / gvu(nm)
                   !
                endif
                !
             enddo
       ...
       do k = 1, kmax
          ...
          do n = nf, nl
             do m = mf+1, mlast
                ...
                numu = kgrpnt(mu,nu)
                !
                fac = 0.25 * ( Vbck(nm,kpv(k)) + Vbck(nmu,kpv(k)) + Vbck(num,kpv(k)) + Vbck(numu,kpv(k)) )
                !
                if ( .not. fac < 0. ) then
                   !
                   ua(nm,k) = u0(nm ,k)
       ...
       enddo
       !
       ! compute advection term in y-direction (momentum conservative) at internal u-point (prediction)
       !
       do k = 1, kmax
          !
          do n = nfu, nl
             ...
             do m = mf+1, mlast
                ...
                numu = kgrpnt(mu,nu)
                !
                fac1 = 0.25 * ( Vbck(nm ,kpv(k)) + Vbck(nmu ,kpv(k)) + Vbck(num,kpv(k)) + Vbck(numu,kpv(k)) )
                fac2 = 0.25 * ( Vbck(ndm,kpv(k)) + Vbck(ndmu,kpv(k)) + Vbck(nm ,kpv(k)) + Vbck(nmu ,kpv(k)) )
                !
                ! to avoid boundary layer effect, set advection near lateral open boundary to zero
                !
                if ( ( .not.lrepty .and. LMYF .and. n == nfu ) .or. ( .not.lrepty .and. LMYL .and. n == nl ) ) then
                ...
                !
                v = 0.25 * ( v0(nm,k) + v0(nmu,k) + v0(ndm,k) + v0(ndmu,k) )
                if ( ( .not.lrepty .and. LMYF .and. n == nfu ) .or. ( .not.lrepty .and. LMYL .and. n == nl ) ) v = 0.
                if ( uwetp(nm) == 1 ) rhsu(nm,k) = rhsu(nm,k) - 0.25 * v * ( Ubck(numu,kpv(k)) + Ubck(num,kpv(k)) - Ubck(ndmu,kpv(k)) - Ubck(ndm,kpv(k)) ) / guu(nm)
                !
             enddo
             !
          enddo
       ...
       do k = 1, kmax
          ...
          do n = nfu, nl
             do m = mfu, ml
                ...
                nmd = kgrpnt(md,n)
                !
                fac = 0.5 * ( Ubck(nm,kpv(k)) + Ubck(nmd,kpv(k)) )
                !
                if ( .not. fac < 0. ) then
                   !
                   ua(nm,k) = u0(nmd,k)
       ...
       do k = 1, kmax
          ...
          do n = nfu, nl
             ...
             do m = mf+1, mlast
                ...
                nmu = kgrpnt(mu,n)
                !
                fac1 = 0.5 * ( Ubck(nmu,kpv(k)) + Ubck(nm ,kpv(k)) )
                fac2 = 0.5 * ( Ubck(nm ,kpv(k)) + Ubck(nmd,kpv(k)) )
                !
                if ( uwetp(nm) == 1 ) then
                   !
                   rhsu(nm,k) = rhsu(nm,k) - ( fac1 * ua(nmu,k) - fac2 * ua(nm,k) ) / gvu(nm)
                   !
                endif
                !
             enddo
       ...
       do k = 1, kmax
          ...
          do n = nf, nl
             do m = mf+1, mlast
                ...
                num = kgrpnt(m ,nu)
                !
                fac = 0.5 * ( Vbck(nm,kpv(k)) + Vbck(nmu,kpv(k)) )
                !
                if ( .not. fac < 0. ) then
                   !
                   ua(nm,k) = u0(nm ,k)
       ...
       do k = 1, kmax
          ...
          do n = nfu, nl
             ...
             do m = mf+1, mlast
                ...
                num  = kgrpnt(m ,nu)
                !
                fac1 = 0.5 * ( Vbck(nm ,kpv(k)) + Vbck(nmu ,kpv(k)) )
                fac2 = 0.5 * ( Vbck(ndm,kpv(k)) + Vbck(ndmu,kpv(k)) )
                !
                ! to avoid boundary layer effect, set advection near lateral open boundary to zero
                !
                if ( ( .not.lrepty .and. LMYF .and. n == nfu ) .or. ( .not.lrepty .and. LMYL .and. n == nl ) ) then
                ...
                v = 0.25 * ( v0(nm,k) + v0(nmu,k) + v0(ndm,k) + v0(ndmu,k) )
                if ( ( .not.lrepty .and. LMYF .and. n == nfu ) .or. ( .not.lrepty .and. LMYL .and. n == nl ) ) v = 0.
                if ( uwetp(nm) == 1 ) rhsu(nm,k) = rhsu(nm,k) - 0.5 * v * ( Ubck(num,kpv(k)) - Ubck(ndm,kpv(k)) ) / guu(nm)
                !
             enddo
             !
          enddo
    ...
    !
    ! include effect of ambient current - high order correction
    !
    propsc = nint(pnums(61))
    !
    if ( iamb == 1 .and. propsc /= 1 ) then
       !
       ! compute higher order correction to the advective velocity in wl-point (if appropriate)
       !
       kappa  = pnums(62)
       ...
       do k = 1, kmax
          ...
          do n = nfu, nl
             do m = mf+1, mlast
                ...
                if ( uwetp(nm) == 1 ) then
                   !
                   u1(nm,k) = u1(nm,k) - dt * ( Ubck(nmu,kpv(k)) * ua(nmu,k)  - Ubck(nm,kpv(k)) * ua(nm,k) ) / gvu(nm)
                   !
                endif
                !
             enddo
       ...
       call periodic ( u1, kgrpnt, 1, kmax )
       !
    else if ( iamb == 2 .and. propsc /= 1 ) then
       !
       ! compute higher order correction to the advective velocity in wl-point (if appropriate)
       !
       kappa  = pnums(62)
    ...
    enddo
    !
    ! compute the mass flux per width in x-direction in depth point based on averaging
    !
    if ( lrepty ) then
    ...
    enddo
    !
    ! compute the mass flux per width in y-direction in wl-point based on averaging
    !
    do k = 1, kmax
       !
       do m = mfu, ml
    ...
    if ( coriolis ) then
       ...
       do m = mfu, ml
          do n = nsta(m), nend(m)
             ...
             numd = kgrpnt(md,nu)
             !
             if ( vwetp(nm) == 1 .and. fcor(nm,2) /= 0. ) then
                !
                ! for permanently dry neighbours, corresponding values will be mirrored
                !
                if ( num  == 1 ) num  = nm
                ...
                ! energy-conserved weighting factors according to Espelid et al. (2000)
                !
                fc(1) = sqrt(abs(fcor(nm  ,1))*dpu(nm  ))
                fc(2) = sqrt(abs(fcor(num ,1))*dpu(num ))
                fc(3) = sqrt(abs(fcor(nmd ,1))*dpu(nmd ))
                fc(4) = sqrt(abs(fcor(numd,1))*dpu(numd))
                fc(5) = 1. / (4.*sqrt(abs(fcor(nm,2))*dpv(nm)))
                !
                do k = 1, kmax
                   !
                   ! at time level n-1
                   ...
                   ! --- modified quasi second order Adams-Bashforth scheme which is conditionally stable for the inviscid case (see Marshall et al., 1997)
                   !
                   rhsu(nm,k) = rhsu(nm,k) + (0.5+epsab2) * cfv(nm,k)
                   !
                   ! at time level n
                   !
                   ! --- interpolate u-velocity at v-point using energy-conserved weighting factors
                   !
                   u = fc(1) * u0(nm,k) + fc(2) * u0(num,k) + fc(3) * u0(nmd,k) + fc(4) * u0(numd,k)
                   u = fc(5) * u
                   !
                   cfv(nm,k) = fcor(nm,2) * u
                   !
                   rhsu(nm,k) = rhsu(nm,k) - (1.5+epsab2) * cfv(nm,k)
                   !
                enddo
                !
             endif
    ...
    if ( iamb == 1 ) then
       ...
       do k = 1, kmax
          ...
          do m = mf, ml
             do n = nf+1, nlast
                ...
                numu = kgrpnt(mu,nu)
                !
                fac = 0.25 * ( Ubck(nm,kpv(k)) + Ubck(num,kpv(k)) + Ubck(nmu,kpv(k)) + Ubck(numu,kpv(k)) )
                !
                if ( .not. fac < 0. ) then
                   !
                   ua(nm,k) = v0(nm ,k)
       ...
       enddo
       !
       ! compute advection term in x-direction (momentum conservative) at internal v-point (prediction)
       !
       do k = 1, kmax
          !
          do m = mfu, ml
             ...
             do n = nf+1, nlast
                ...
                numu = kgrpnt(mu,nu)
                !
                fac1 = 0.25 * ( Ubck(nm ,kpv(k)) + Ubck(num ,kpv(k)) + Ubck(nmu,kpv(k)) + Ubck(numu,kpv(k)) )
                fac2 = 0.25 * ( Ubck(nmd,kpv(k)) + Ubck(numd,kpv(k)) + Ubck(nm ,kpv(k)) + Ubck(num ,kpv(k)) )
                !
                ! to avoid boundary layer effect, set advection near lateral open boundary to zero
                !
                if ( ( .not.lreptx .and. LMXF .and. m == mfu ) .or. ( .not.lreptx .and. LMXL .and. m == ml ) ) then
                ...
                !
                u = 0.25 * ( u0(nm,k) + u0(num,k) + u0(nmd,k) + u0(numd,k) )
                if ( ( .not.lreptx .and. LMXF .and. m == mfu ) .or. ( .not.lreptx .and. LMXL .and. m == ml ) ) u = 0.
                if ( vwetp(nm) == 1 ) rhsu(nm,k) = rhsu(nm,k) - 0.25 * u * ( Vbck(numu,kpv(k)) + Vbck(nmu,kpv(k)) - Vbck(numd,kpv(k)) - Vbck(nmd,kpv(k)) ) / gvv(nm)
                !
             enddo
             !
          enddo
       ...
       do k = 1, kmax
          ...
          do m = mfu, ml
             ...
             do n = nf+1, nlast
                ...
                if ( vwetp(nm) == 1 ) then
                   !
                   rhsu(nm,k) = rhsu(nm,k) - ( Vbck(num,kpv(k)) * ua(num,k) - Vbck(nm,kpv(k)) * ua(nm,k) ) / guv(nm)
                   !
                endif
                !
             enddo
       ...
       do k = 1, kmax
          ...
          do m = mf, ml
             do n = nf+1, nlast
                ...
                num = kgrpnt(m ,nu)
                !
                fac = 0.5 * ( Ubck(nm,kpv(k)) + Ubck(num,kpv(k)) )
                !
                if ( .not. fac < 0. ) then
                   !
                   ua(nm,k) = v0(nm ,k)
       ...
       do k = 1, kmax
          ...
          do m = mfu, ml
             ...
             do n = nf+1, nlast
                ...
                numd = kgrpnt(md,nu)
                !
                fac1 = 0.5 * ( Ubck(nm ,kpv(k)) + Ubck(num ,kpv(k)) )
                fac2 = 0.5 * ( Ubck(nmd,kpv(k)) + Ubck(numd,kpv(k)) )
                !
                ! to avoid boundary layer effect, set advection near lateral open boundary to zero
                !
                if ( ( .not.lreptx .and. LMXF .and. m == mfu ) .or. ( .not.lreptx .and. LMXL .and. m == ml ) ) then
                ...
                u = 0.25 * ( u0(nm,k) + u0(num,k) + u0(nmd,k) + u0(numd,k) )
                if ( ( .not.lreptx .and. LMXF .and. m == mfu ) .or. ( .not.lreptx .and. LMXL .and. m == ml ) ) u = 0.
                if ( vwetp(nm) == 1 ) rhsu(nm,k) = rhsu(nm,k) - 0.5 * u * ( Vbck(nmu,kpv(k)) - Vbck(nmd,kpv(k)) ) / gvv(nm)
                !
             enddo
             !
          enddo
       ...
       do k = 1, kmax
          ...
          do m = mfu, ml
             do n = nfu, nl
                ...
                ndm = kgrpnt(m,nd)
                !
                fac = 0.5 * ( Vbck(nm,kpv(k)) + Vbck(ndm,kpv(k)) )
                !
                if ( .not. fac < 0. ) then
                   !
                   ua(nm,k) = v0(ndm,k)
       ...
       do k = 1, kmax
          ...
          do m = mfu, ml
             ...
             do n = nf+1, nlast
                ...
                num = kgrpnt(m,nu)
                !
                fac1 = 0.5 * ( Vbck(num,kpv(k)) + Vbck(nm ,kpv(k)) )
                fac2 = 0.5 * ( Vbck(nm ,kpv(k)) + Vbck(ndm,kpv(k)) )
                !
                if ( vwetp(nm) == 1 ) then
                   !
                   rhsu(nm,k) = rhsu(nm,k) - ( fac1 * ua(num,k) - fac2 * ua(nm,k) ) / guv(nm)
                   !
                endif
                !
             enddo
       ...
       do k = 1, kmax
          ...
          do m = mfu, ml
             do n = nf+1, nlast
                ...
                if ( vwetp(nm) == 1 ) then
                   !
                   v1(nm,k) = v1(nm,k) - dt * ( Vbck(num,kpv(k)) * ua(num,k)  - Vbck(nm,kpv(k)) * ua(nm,k) ) / guv(nm)
                   !
                endif
                !
             enddo
             ...
             do n = nfu, nl
                do m = mfu, ml
                   ...
                   if ( wets(nm) == 1 ) then
                      ...
                      !
                      if ( iamb == 1 ) then
                         fac1 = 0.25 * ( hkuc(nmd,kd)*(Ubck(nm ,kd) + Ubck(nmd,kd)) + hkuc(nmd,k+1)*(Ubck(nm ,k+1) + Ubck(nmd,k+1)) )
                         fac2 = 0.25 * ( hkuc(nm ,kd)*(Ubck(nmu,kd) + Ubck(nm ,kd)) + hkuc(nm ,k+1)*(Ubck(nmu,k+1) + Ubck(nm ,k+1)) )
                      else if ( iamb == 2 ) then
                         fac1 = 0.5 * ( hkuc(nmd,kd)*Ubck(nmd,kd) + hkuc(nmd,k+1)*Ubck(nmd,k+1) )
                         fac2 = 0.5 * ( hkuc(nm ,kd)*Ubck(nm ,kd) + hkuc(nm ,k+1)*Ubck(nm ,k+1) )
                      endif
                      fac1 = guu(nmd) * fac1 / fac
                      fac2 = guu(nm ) * fac2 / fac
                      !
                      if ( propsc == 3 .and. kappa == 1. ) then
                         !
                         ! central differences
                      ...
                      !
                      if ( iamb == 1 ) then
                         fac1 = 0.25 * ( hkvc(ndm,kd)*(Vbck(nm ,kd) + Vbck(ndm,kd)) + hkvc(ndm,k+1)*(Vbck(nm ,k+1) + Vbck(ndm,k+1)) )
                         fac2 = 0.25 * ( hkvc(nm ,kd)*(Vbck(num,kd) + Vbck(nm ,kd)) + hkvc(nm ,k+1)*(Vbck(num,k+1) + Vbck(nm ,k+1)) )
                      else if ( iamb == 2 ) then
                         fac1 = 0.5 * ( hkvc(ndm,kd)*Vbck(ndm,kd) + hkvc(ndm,k+1)*Vbck(ndm,k+1) )
                         fac2 = 0.5 * ( hkvc(nm ,kd)*Vbck(nm ,kd) + hkvc(nm ,k+1)*Vbck(nm ,k+1) )
                      endif
                      fac1 = gvv(ndm) * fac1 / fac
                      fac2 = gvv(nm ) * fac2 / fac
                      !
                      if ( propsc == 3 .and. kappa == 1. ) then
                         !
                         ! central differences
          ...
          do n = nfu, nl
             do m = mfu, ml
                ...
                if ( wets(nm) == 1 ) then
                   ...
                   !
                   if ( iamb == 1 ) then
                      fac1 = 0.5 * ( Ubck(nm ,1) + Ubck(nmd,1) )
                      fac2 = 0.5 * ( Ubck(nmu,1) + Ubck(nm ,1) )
                   else if ( iamb == 2 ) then
                      fac1 = Ubck(nmd,1)
                      fac2 = Ubck(nm ,1)
                   endif
                   fac1 = fac * guu(nmd) * hu(nmd) * fac1 / hs(nm)
                   fac2 = fac * guu(nm ) * hu(nm ) * fac2 / hs(nm)
                   !
                   if ( propsc == 3 .and. kappa == 1. ) then
                      !
                      ! central differences
                   ...
                   !
                   if ( iamb == 1 ) then
                      fac1 = 0.5 * ( Vbck(nm ,1) + Vbck(ndm,1) )
                      fac2 = 0.5 * ( Vbck(num,1) + Vbck(nm ,1) )
                   else if ( iamb == 2 ) then
                      fac1 = Vbck(ndm,1)
                      fac2 = Vbck(nm ,1)
                   endif
                   fac1 = fac * gvv(ndm) * hv(ndm) * fac1 / hs(nm)
                   fac2 = fac * gvv(nm ) * hv(nm ) * fac2 / hs(nm)
                   !
                   if ( propsc == 3 .and. kappa == 1. ) then
                      !
                      ! central differences
...
!TIMG          call SWTSTO(297)
    ...
    !
    ! store layer interfaces in velocity points
    !
    zkuo = zku
    zkvo = zkv
    !
    ...
    if ( LMYL ) nend(1) = nl
    !
    ! solve global continuity equation by means of a second order predictor-corrector scheme
    ...
    do j = jhk, 3
       ...
       !
       ! compute the mass flux per layer in x-direction
       !
       do k = 1, kmax
          !
          do n = nfu, nl
       ...
       enddo
       !
       ! compute the mass flux per layer in y-direction
       !
       do k = 1, kmax
          !
          do n = nf, nl
       ...
       enddo
       !
       ! compute the depth-integrated mass fluxes
       !
       if ( ihydro /= 3 ) then
          !
          work(:,1) = 0.
          ...
          if ( j == jhk ) then
             ...
             do n = nfu, nl
                do m = mf, ml
                   ...
                   if ( uwetp(nm) == 1 ) then
                      !
                      udep(nm) = work(nm,1) / ( hu(nm)*guu(nm) )
                      !
                   endif
                   !
                enddo
             ...
             do m = mfu, ml
                do n = nf, nl
                   ...
                   if ( vwetp(nm) == 1 ) then
                      !
                      vdep(nm) = work(nm,2) / ( hv(nm)*gvv(nm) )
                      !
                   endif
                   !
                enddo
          ...
          do n = nfu, nl
             do m = mf, ml
                ...
                nm = kgrpnt(m,n)
                !
                work(nm,1) = hu(nm) * guu(nm) * udep(nm)
                !
             enddo
          enddo
          !
          do n = nf, nl
             do m = mfu, ml
                ...
                nm = kgrpnt(m,n)
                !
                work(nm,2) = hv(nm) * gvv(nm) * vdep(nm)
                !
             enddo
          enddo
          !
       ...
       do n = nfu, nend(1)
          do m = mfu, mend(1)
             ...
             ndm = kgrpnt(m ,nd)
             !
             s1(nm) = s0(nm) - fh * dt * ( work(nm,1) - work(nmd,1) + work(nm,2) - work(ndm,2) ) / gsqs(nm)
             !
          enddo
       enddo
       !
       ...
       if ( j <= 2 ) then
          !
          ! correct the water depths for mass fluxes
          !
          call SwashUpdDepu ( udep, vdep )
          !
          ! recompute layer interfaces and layer thicknesses at velocity points
          !
          zku(:,0) = zku(:,kmax) + hu(:)
          call sigmacoor( zku, hu, mcgrd )
          !
          do k = 1, kmax
             hku(:,k) = zku(:,k-1) - zku(:,k)
          enddo
    ...
    !
    ! include effect of ambient current
    !
    if ( iamb == 1 ) then
       !
       so = s1
       !
       ! solve global continuity equation by means of the Heun method
       !
       do j = 1, 2
          !
          work(:,2) = 0.
          ...
          if ( j == 1 ) then
             ...
             do n = nfu, nend(1)
                do m = mfu, mend(1)
                   ...
                   num = kgrpnt(m ,nu)
                   !
                   work(nm,2) = work(nm,2) + ( Ubck(nmu,1) + Ubck(nm,1) ) * guu(nm) * ( so(nmu) + so(nm) ) - ( Ubck(nm,1) + Ubck(nmd,1) ) * guu(nmd) * ( so(nm) + so(nmd) )
                   work(nm,2) = work(nm,2) + ( Vbck(num,1) + Vbck(nm,1) ) * gvv(nm) * ( so(num) + so(nm) ) - ( Vbck(nm,1) + Vbck(ndm,1) ) * gvv(ndm) * ( so(nm) + so(ndm) )
                   work(nm,2) = 0.25 * work(nm,2)
                   !
                enddo
             enddo
             !
             ...
             do n = nfu, nend(1)
                do m = mfu, mend(1)
                   ...
                   num = kgrpnt(m ,nu)
                   !
                   fac1 = thetamb * s1(nmd) + (1.-thetamb) * so(nmd)
                   fac  = thetamb * s1(nm ) + (1.-thetamb) * so(nm )
                   fac2 = thetamb * s1(nmu) + (1.-thetamb) * so(nmu)
                   !
                   work(nm,2) = work(nm,2) + ( Ubck(nmu,1) + Ubck(nm,1) ) * guu(nm) * ( fac2 + fac ) - ( Ubck(nm,1) + Ubck(nmd,1) ) * guu(nmd) * ( fac + fac1 )
                   !
                   fac1 = thetamb * s1(ndm) + (1.-thetamb) * so(ndm)
                   fac2 = thetamb * s1(num) + (1.-thetamb) * so(num)
                   !
                   work(nm,2) = work(nm,2) + ( Vbck(num,1) + Vbck(nm,1) ) * gvv(nm) * ( fac2 + fac ) - ( Vbck(nm,1) + Vbck(ndm,1) ) * gvv(ndm) * ( fac + fac1 )
                   !
                   work(nm,2) = 0.25 * work(nm,2)
                   !
                enddo
          ...
          do n = nfu, nend(1)
             do m = mfu, mend(1)
                ...
                nm = kgrpnt(m,n)
                !
                s1(nm) = so(nm) - dt * work(nm,2) / gsqs(nm)
                !
             enddo
          enddo
          !
          ...
          if ( j == 1 ) then
             ...
             do n = nfu, nend(1)
                do m = mfu, mend(1)
                   ...
                   num = kgrpnt(m ,nu)
                   !
                   work(nm,2) = work(nm,2) + Ubck(nm,1) * guu(nm) * ( so(nmu) + so(nm) ) - Ubck(nmd,1) * guu(nmd) * ( so(nm) + so(nmd) )
                   work(nm,2) = work(nm,2) + Vbck(nm,1) * gvv(nm) * ( so(num) + so(nm) ) - Vbck(ndm,1) * gvv(ndm) * ( so(nm) + so(ndm) )
                   work(nm,2) = 0.5 * work(nm,2)
                   !
                enddo
             enddo
             !
             ...
             do n = nfu, nend(1)
                do m = mfu, mend(1)
                   ...
                   fac2 = thetamb * s1(nmu) + (1.-thetamb) * so(nmu)
                   !
                   work(nm,2) = work(nm,2) + Ubck(nm,1) * guu(nm) * ( fac2 + fac ) - Ubck(nmd,1) * guu(nmd) * ( fac + fac1 )
                   !
                   fac1 = thetamb * s1(ndm) + (1.-thetamb) * so(ndm)
                   fac2 = thetamb * s1(num) + (1.-thetamb) * so(num)
                   !
                   work(nm,2) = work(nm,2) + Vbck(nm,1) * gvv(nm) * ( fac2 + fac ) - Vbck(ndm,1) * gvv(ndm) * ( fac + fac1 )
                   !
                   work(nm,2) = 0.5 * work(nm,2)
                   !
                enddo
          ...
          do m = mfu, mend(1)
             ...
             if ( wets(nm) == 1 ) then
                ...
                do k = 1, kpmax-1
                   ...
                   !
                   womp(nm,k) = w1p(nm,k) - ( zksnew(nm,kup(k)) - zks(nm,kup(k)) ) / dt - u * ( zkuo(nm,kup(k)) - zkuo(nmd,kup(k)) ) / dxl  &
                                                                                        - v * ( zkvo(nm,kup(k)) - zkvo(ndm,kup(k)) ) / dyl
                   !
                enddo
                !
                womp(nm,0    ) = 0.
                ...
                do kp = kpmax, 1, -1
                   ...
                   do j = npu(kp), 1, -1
                      ...
                      if ( j == npu(kp) ) then
                         ...
                         !
                         w1(nm,k) = wom(nm,k) + ( zksnew(nm,k) - zks(nm,k) ) / dt + u * ( zkuo(nm,k) - zkuo(nmd,k) ) / dxl  &
                                                                                  + v * ( zkvo(nm,k) - zkvo(ndm,k) ) / dyl
                         !
                      endif
                      !
                   enddo
