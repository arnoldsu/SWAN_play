...
if ($os =~ /IRIX64/i) {
  ...
  if ( $compiler eq "ifx" )
  {
    ...
    print OUTFILE "################################################################################\n";
    print OUTFILE "# for older compiler versions, use ifort\n";
    print OUTFILE "F90_SER = ifx\n";
    print OUTFILE "F90_OMP = ifx\n";
    print OUTFILE "# for older compiler versions, use mpif90 or mpiifort\n";
...
#    print OUTFILE "LIBS_MPI = C:\\\PROGRA~1\\\MPICH2\\\llib\\\lfmpich2.lib\n";
  ...
  elsif ( $compiler eq "gfortran.exe" )
  {
    my $ver = `gcc -dumpversion`;
    print OUTFILE "###############################################################################\n";
    print OUTFILE "# IA32_GNU/EM64T_GNU:	Intel with MS Windows using GNU compiler gfortran.\n";
    print OUTFILE "# Note: it is assumed that strawberryperl.com has been installed!\n";
    print OUTFILE "###############################################################################\n";
    print OUTFILE "F90_SER = gfortran\n";
    print OUTFILE "F90_OMP = gfortran\n";
    print OUTFILE "F90_MPI = mpifort\n";
...
sub getcmpl {
   ...
   unless ( $compiler ) {
      foreach ('ifx','gfortran','f90','ifc','efc','pgf90','xlf90', 'lf95','g95','ifx.exe','gfortran.exe','ifort','ifort.exe') {
         $compiler = $_;
